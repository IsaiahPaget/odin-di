...
make_system_0 :: proc(f: proc()) -> StoredSystem {
	...
	run_proc := proc(system: ^System, resources: ^ResourceMap) {
		fmt.assertf(system != nil, "scheduler is nil")
		...
		parent := cast(^FunctionSystem(proc()))system // pointer reinterpretation
...
make_system_1 :: proc($T1: typeid, f: proc(_: T1)) -> ^System {
	fs := new(FunctionSystem(proc(_: T1)))
	fs.func = f
...
make_system_2 :: proc($T1: typeid, $T2: typeid, f: proc(_: T1, _: T2)) -> ^System {
	fs := new(FunctionSystem(proc(_: T1, _: T2)))
	fs.func = f
...
add_system_0 :: proc(scheduler: ^Scheduler, system: proc()) {
...
add_system_1 :: proc(scheduler: ^Scheduler, $T1: typeid, system: proc(_: T1)) {
	fmt.assertf(scheduler != nil, "scheduler is nil")
	fmt.assertf(system != nil, "system is nil")
	sa.append(&scheduler.systems, make_system(T1, system))
}

add_system_2 :: proc(scheduler: ^Scheduler, $T1, $T2: typeid, system: proc(_: T1, _: T2)) {
	fmt.assertf(scheduler != nil, "scheduler is nil")
	fmt.assertf(system != nil, "system is nil")
